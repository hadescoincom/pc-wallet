...
class SendSwapViewModel: public QObject
{
    ...
    Q_PROPERTY(bool          isSendHds       READ isSendHds                               NOTIFY tokenChanged)
...
public slots:
    void onChangeCalculated(hds::Amount change);

private:
    void fillParameters(const hds::wallet::TxParameters& parameters);
    void recalcAvailable();

    hds::Amount _sendAmountGrothes;
    hds::Amount _sendFeeGrothes;
    Currency     _sendCurrency;
    hds::Amount _receiveAmountGrothes;
    hds::Amount _receiveFeeGrothes;
    Currency     _receiveCurrency;
    hds::Amount _changeGrothes;

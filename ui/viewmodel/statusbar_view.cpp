...

{
    ...
    connect(&m_model, SIGNAL(walletError(hds::wallet::ErrorType)),
        SLOT(onGetWalletError(hds::wallet::ErrorType)));
    ...
    connect(&AppModel::getInstance().getNode(), SIGNAL(failedToSyncNode(hds::wallet::ErrorType)),
            SLOT(onGetWalletError(hds::wallet::ErrorType)));
...
void StatusbarViewModel::onGetWalletError(hds::wallet::ErrorType error)
{
    setIsOnline(false);

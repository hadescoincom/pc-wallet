...
{
    return AmountToUIString(m_paymentInfo.m_Amount, Currencies::Hds);
}
...
void PaymentInfoItem::setPaymentProof(const QString& value)
{
    if (m_paymentProof != value)
    {
        ...
        try
        {
            m_paymentInfo = hds::wallet::storage::PaymentInfo::FromByteBuffer(hds::from_hex(m_paymentProof.toStdString()));
        ...
        : PaymentInfoItem(parent)
{
    ...
    connect(model.get(), SIGNAL(paymentProofExported(const hds::wallet::TxID&, const QString&)), SLOT(onPaymentProofExported(const hds::wallet::TxID&, const QString&)));
    model->getAsync()->exportPaymentProof(txID);
}

void MyPaymentInfoItem::onPaymentProofExported(const hds::wallet::TxID& txID, const QString& proof)
{

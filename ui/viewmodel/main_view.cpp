    ...
    , m_timer(this)
{
    ...
    connect(walletPtr, SIGNAL(transactionsChanged(hds::wallet::ChangeAction, const std::vector<hds::wallet::TxDescription>&)), SIGNAL(unsafeTxCountChanged()));
    connect(walletPtr, SIGNAL(notificationsChanged(hds::wallet::ChangeAction, const std::vector<hds::wallet::Notification>&)), SIGNAL(unreadNotificationsChanged()));

...
QR::QR(const QString& addr, uint width, uint height, hds::Amount amount)
    : m_addr(addr)
    , m_width(width)
    , m_height(height)
...
void QR::setAmount(hds::Amount amount)
{
    m_amountGrothes = amount;
    update();
}
...
void QR::update()
{
    ...
    {
        query.addQueryItem("amount", hdsui::AmountToUIString(m_amountGrothes));
    }

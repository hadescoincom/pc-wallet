...
void TrezorThread::run()
{
    ...
    hds::wallet::IPrivateKeyKeeper2::Method::get_Kdf m;
    m.m_Root = true;
...
const QList<QObject*>& StartViewModel::getRecoveryPhrases()
{
    if (m_recoveryPhrases.empty())
    {
        if (!m_isRecoveryMode)
        {
            m_generatedPhrases = hds::createMnemonic(hds::getEntropy(), hds::language::en);
        }
        else
        {
...
void StartViewModel::createWallet(const QJSValue& callback)
{
    ...
    auto buf = hds::decodeMnemonic(m_generatedPhrases);

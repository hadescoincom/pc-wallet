...
LoadingViewModel::LoadingViewModel()
    ...
    , m_lastProgress{0.}
    , m_bpsWholeTimeFilter(std::make_unique<hdsui::Filter>(kFilterRange))
    , m_bpsWindowedFilter(std::make_unique<hdsui::Filter>(kFilterRange * 3))
    , m_estimateFilter(std::make_unique<hdsui::Filter>(kFilterRange))
    , m_startTimestamp{0}
    ...
    , m_bpsRecessionCount{0}
{
    ...
    connect(&m_walletModel, SIGNAL(walletError(hds::wallet::ErrorType)), SLOT(onGetWalletError(hds::wallet::ErrorType)));
...
void LoadingViewModel::resetWallet()
{
    ...
    disconnect(&m_walletModel, SIGNAL(walletError(hds::wallet::ErrorType)), this, SLOT(onGetWalletError(hds::wallet::ErrorType)));
...
void LoadingViewModel::updateProgress()
{
    ...
    if (m_isDownloadStarted)
    {
        ...
        else
        {
            ...
            estimateStr = QString::asprintf(
                    estimateStr.toStdString().c_str(),
                    hdsui::getEstimateTimeStr(m_estimate).toStdString().c_str());
        }
...
void LoadingViewModel::onGetWalletError(hds::wallet::ErrorType error)
{
    if (m_isCreating)
    {
        switch (error)
        {
            case hds::wallet::ErrorType::NodeProtocolIncompatible:
            {
                //% "Incompatible peer"
                emit walletError(qtTrId("loading-view-protocol-error"), m_walletModel.GetErrorString(error));
                return;
            }
            case hds::wallet::ErrorType::ConnectionAddrInUse:
            case hds::wallet::ErrorType::ConnectionRefused:
            case hds::wallet::ErrorType::HostResolvedError:
            {
                //% "Connection error"

...
ConfirmationDialog {
    ...
    readonly property string    currencyLabel:  HdsGlobals.getCurrencyLabel(sendViewConfirm.currency)
    readonly property string    feeLabel:       !sendViewConfirm.swapMode
                                                //% "Transaction fee"
                                                ? (qsTrId("general-fee") + ":")
                                                : (sendViewConfirm.currency == Currency.CurrHds
                                                    //% "HDS Transaction fee"
                                                    ? (qsTrId("hds-transaction-fee") + ":")
                                                    //% "%1 Transaction fee rate"
                                                    : qsTrId("general-fee-rate").arg(currencyLabel))
    property Item   defaultFocusItem:   HdsGlobals.needPasswordToSpend() ? requirePasswordInput : cancelButton
    ...
    okButtonEnable:          HdsGlobals.needPasswordToSpend() ? requirePasswordInput.text.length : true
    cancelButtonIconSource:  "qrc:/assets/icon-cancel-white.svg"

    function confirmationHandler() {
        if (HdsGlobals.needPasswordToSpend()) {
            if (requirePasswordInput.text.length == 0) {
                requirePasswordInput.forceActiveFocus(Qt.TabFocusReason);
                return;
            }
            if (!HdsGlobals.isPasswordValid(requirePasswordInput.text)) {
                requirePasswordInput.forceActiveFocus(Qt.TabFocusReason);
        ...
        ColumnLayout {
            ...
            GridLayout {
                ...
                ColumnLayout {
                    ...
                    SFText {
                        ...
                        text: [
                                ...
                                HdsGlobals.getFeeRateLabel(sendViewConfirm.currency)
                            ].join(" ")
                    }
                    SFText {
                ...
                SFText {
                    ...
                    visible: HdsGlobals.needPasswordToSpend()
                    Layout.row: 4
                    Layout.columnSpan: 2
                    Layout.topMargin: 50
                ...
                SFText {
                    ...
                    text: sendViewConfirm.swapMode ?
                        ...
                        qsTrId("send-swap-sconfirmation-online-time") + (sendViewConfirm.currency !== Currency.CurrHds ?

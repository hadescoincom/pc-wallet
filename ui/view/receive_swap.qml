...
ColumnLayout {
    ...
    function currencyError() {
        ...
        if (viewModel.receiveCurrency != Currency.CurrHds && viewModel.sentCurrency != Currency.CurrHds) return true;
        return false;
    }
    ...
    Component.onCompleted: {
        if (!HdsGlobals.canSwap()) swapna.open();
    }
            ...
            Grid {
                ...
                ColumnLayout {
                    ...
                    AmountInput {
                        ...
                        multi:            true
                        resetAmount:      false
                        currColor:        currencyError() || !HdsGlobals.canReceive(currency) ? Style.validator_error : Style.content_main
                        error:            getErrorText()

                        function getErrorText() {
                            if(!HdsGlobals.canReceive(currency)) {
/*% "%1 is not connected, 
please review your settings and try again"
*/
                                return qsTrId("swap-currency-na-message").arg(HdsGlobals.getCurrencyName(currency)).replace("\n", "")
                            }
                        ...
                        onCurrencyChanged: {
                            if(sentAmountInput.currency != Currency.CurrHds) {
                                if(receiveAmountInput.currency != Currency.CurrHds) {
                                    receiveAmountInput.currency = Currency.CurrHds
                                }
                            }
                        }
                    }
...
*/
                        ...
                        onCurrencyChanged: {
                            if(receiveAmountInput.currency != Currency.CurrHds) {
                                if(sentAmountInput.currency != Currency.CurrHds) {
                                    sentAmountInput.currency = Currency.CurrHds
                                }
                            }
                        }
                    }
                    ...
                    RowLayout {
                        ...
                        function changeReceive(byRate) {
                            ...
                            if (sentAmountInput.amount != "0" && rateValue) {
                                receiveAmountInput.amount= viewModel.isSendHds
                                    ? HdsGlobals.multiplyWithPrecision8(sentAmountInput.amount, rateValue)
                                    : HdsGlobals.divideWithPrecision8(sentAmountInput.amount, rateValue);
                        ...
                        SFText {
                            font.pixelSize:  14
                            color:           rateRow.rateValid ? Style.content_secondary : Style.validator_error
                            text:            viewModel.isSendHds ? receiveAmountInput.currencyLabel : sentAmountInput.currencyLabel
                        }
                    }

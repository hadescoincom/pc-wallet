...
# Notarize the dmg
notarize_dmg() {(
  echo "Uploading $HDS_WALLET_UI_IN to notarization service"
  uploadRes=$(xcrun altool --notarize-app --primary-bundle-id "com.mw.hds.hdswallet" --username "$MACOS_NOTARIZE_USER" --password "$MACOS_NOTARIZE_PASS" --asc-provider "$MACOS_NOTARIZE_PROVIDER" --file "$HDS_WALLET_UI_IN" 2>&1)
  ...
  do
    ...
    if [ "$status" = "success" ]; then
      xcrun stapler staple "$HDS_WALLET_UI_IN"
